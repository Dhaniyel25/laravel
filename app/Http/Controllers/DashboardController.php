...
class DashboardController extends Controller
{
    ...
    public function prosesResetPassword(Request $request){
        ...
        if (Hash::check($old_password, $user->password)){
            ...
            try {
                $user->save();
                return redirect(route('dashboard.resetPassword'))->with('pesan',['success','Selamat Anda Berhasil Rubah Password']);
            }catch (\Exception $e){
                return redirect(route('dashboard.resetPassword'))->with('pesan',['danger',' Anda Gagal Rubah Password']);
